...
func (st *Storage) addStudentsGroup(group *models.StudentGroup) error {
	...
	_, err = st.Db.Exec("INSERT INTO student_groups (id, grade, institute, name, students) VALUES ($1,$2,$3,$4,$5)", group.Id, group.Grade, group.Institute, group.Name, jsonstudents)
...
func (st *Storage) getGroupByGroupID(id *int) (*models.StudentGroup, error) {
	...
	err := st.Db.QueryRow("SELECT grade, institute, name, students FROM student_groups WHERE id = $1", id).
